...
def login_to_host(host_url: str, username: str, password: str, inbound_id: int) -> tuple[Api | None, Inbound | None]:
    try:
        ...
        if target_inbound is None:
            available_ids = [inbound.id for inbound in inbounds]
            logger.error(f"Inbound with ID '{inbound_id}' not found on host '{host_url}'. Available IDs: {available_ids}")
            logger.error(f"Hint: For new 3x-ui installations, the first inbound typically has ID=1")
            return api, None
...
async def create_key_with_load_balancing(email: str, days_to_add: int) -> Dict | None:
    """Создает ключ, используя балансировщик для выбора оптимального хоста"""
    
    # Получаем оптимальный хост с помощью балансировщика
    optimal_host = LoadBalancer.get_optimal_host()
    if not optimal_host:
        logger.error("No available hosts found for key creation")
        return None
    
    host_name = optimal_host['host_name']
    
    # Проверяем доступность хоста
    if not LoadBalancer.check_host_availability(host_name):
        logger.error(f"Host '{host_name}' is not available (at capacity limit)")
        return None
    
    # Создаем ключ на выбранном хосте
    result = await create_or_update_key_on_host(host_name, email, days_to_add)
    
    if result:
        # Увеличиваем счетчик конфигураций для хоста
        LoadBalancer.allocate_config_to_host(host_name)
        logger.info(f"Successfully created key on host '{host_name}' using load balancer")
    else:
...
async def delete_client_on_host(host_name: str, client_email: str) -> bool:
    ...
    try:
        ...
        if client_to_delete:
            ...
            # Уменьшаем счетчик конфигураций при удалении клиента
            LoadBalancer.release_config_from_host(host_name)
